...
# models = {"Model":[],"Thickness":[],"Length":[]} # add model id in the dictionary 'models with key ["Model"]
...
for ii in range(1,Iterations+1):
    ...
    length = 250                                                                                            ### CHANGE
    # thickness = 2                                                                                           ### CHANGE
    # velocity = 8000                                                                                         ### CHANGE
    # Impactor_mass = 0.25                                                                                    ### Can change but keeping contant for now
    
    # width = random.choice(np.linspace(40, 80, num= 9, endpoint=True))                                         ### CHANGING like 40, 45, 50, ...
    # height = width                                                                                            ### height CHANGING with width, thus square cross section
    # length = random.choice(np.linspace(150, 300, num= 31, endpoint=True))                                     ### CHANGING like 150, 160, 170,  ...
    ...
    # cannot use pandas in python 2.7 in Abaqus 
    # (Pandas don't work in Abaqus command interface) Save the dictory of models with width and height for every model
    ...
    # Keeping the cross-section of the CB square of 100 mm and constant for now and achanged to the random variables in the next few lines
    ...
    # myModel.keywordBlock.replace(78,'\n** --------------------------09--------------------------------------\n** \n*IMPERFECTION,FILE='+eigen_jobs[ii-1]+',STEP=1\n1,'+imperfection_scale)
    # myModel.keywordBlock.replace(78,'\n** ----------------------------------------------------------------\n** \n*IMPERFECTION,FILE=Eigen_Job-1,STEP=1\n1,0.1')
    ...
    # region_RP_Im = odb.rootAssembly.instances['IMPACTOR_PLATE-1'].nodeSets['RP_IM']
    # region_CB_front_surf = myModel.rootAssembly.Surface['CB_Solid_Front_Surf']
    # region_RP_CB = myModel.rootAssembly.allInstances['Crashbox_Solid-1'].sets['RP_CB_Solid']
    ...
    # U3_PI = np.array(odb.steps[step].historyRegions[region_RP_CB].historyOutputs['U3'].data)
    
    # # Displacment of RP impactor but take care of the number of node in impactor
    # U3_PI = np.array(session.XYDataFromHistory(name='U3 PI: IMPACTOR_PLATE-1 Node 243 in NSET RP_IM', odb=odb, 
    # outputVariableName='Spatial displacement: U3 PI: IMPACTOR_PLATE-1 Node 243 in NSET RP_IM', steps=('Loading', ), __linkedVpName__='Viewport: 1'))
    
    # #################### Displacement of CB cannot easily transfer the data from RP CB sets as the node number changes and get a error #######################
    # # Displacment of RP CB solid
    # U3_PI = np.array(session.XYDataFromHistory(name='U3 PI: CRASHBOX_SOLID-1 Node 6561 in NSET RP_CB_SOLID', odb=odb, 
    # outputVariableName='Spatial displacement: U3 PI: CRASHBOX_SOLID-1 Node 1849 in NSET RP_CB_SOLID', steps=('Loading', ), suppressQuery=True, __linkedVpName__='Viewport: 1'))
    
    # # Displacment of RP CB shell
    # U3_PI = np.array(session.XYDataFromHistory(name='U3 PI: CRASHBOX_SHELL-1 Node 3281 in NSET RP_CB_SHELL', odb=odb, 
    # outputVariableName='Spatial displacement: U3 PI: CRASHBOX_SHELL-1 Node 3281 in NSET RP_CB_SHELL', steps=('Loading', ), suppressQuery=True, __linkedVpName__='Viewport: 1')
    
    # # CNF in CB Solid front
    # ContactForce = np.array(session.XYDataFromHistory(name='Total force due to contact pressure: CFNM on surface ASSEMBLY_CB_SOLID_FRONT_SURF', odb=odb, 
    # outputVariableName='Total force due to contact pressure: CFNM on surface ASSEMBLY_CB_SOLID_FRONT_SURF', steps=('Loading', ), suppressQuery=True, __linkedVpName__='Viewport: 1'))
    
    # # CNF in CB shell front
    # ContactForce = np.array(session.XYDataFromHistory(name='Total force due to contact pressure: CFNM on surface ASSEMBLY_CB_SHELL_FRONT_SURF', odb=odb, 
    # outputVariableName='Total force due to contact pressure: CFNM on surface ASSEMBLY_CB_SHELL_FRONT_SURF', steps=('Loading', ), __linkedVpName__='Viewport: 1')) 
    
    # ContactForce = np.array(odb.steps[step].historyRegions['ElementSet  Phis  IBATCH'].historyOutputs['CFNM on surface ASSEMBLY_CB_SOLID_FRONT_SURF'].data)
    ...
    # # # Mass of crashbox
    # # CB_mass = myModel.parts['Crashbox_Solid'].getMassProperties()['mass'] ### Check mass defination of for the solid cb
    ...
    # CB_mass = np.array(session.XYDataFromHistory(name='Total force due to contact pressure: CFNM on surface ASSEMBLY_CB_SOLID_FRONT_SURF',
    # odb=odb, outputVariableName='Mass: MASS  of element set ASSEMBLY_CRASHBOX_SHELL-1_CB_SHELL_BODY', steps=('Loading', ), suppressQuery=True, __linkedVpName__='Viewport: 1'))
    ...
    # np.savetxt(Job_id + '_Energies_RF_U' '.csv', HO, delimiter = ',', fmt = '%f')
